...
let
  ...
  barbicanConf = pkgs.writeText "barbican-api.conf" ''
    ...
    # Test-only KEK: base64("0123456789abcdef0123456789abcdef").
    kek = MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=
  '';
in
{
  options.barbican = {
    ...
    barbicanPackage = mkOption {
      default = barbican;
      type = types.package;
      description = ''
        The OpenStack Barbican package to use.
      '';
    };
  };
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.settings = {
      "10-barbican" = {
        ...
        "/etc/barbican/barbican-api-paste.ini" = {
          L = {
            argument = "${cfg.barbicanPackage}/etc/barbican/barbican-api-paste.ini";
          };
        };
    ...
    systemd.services.barbican-worker = {
      ...
      requires = [ "barbican.service" ];
      path = [ cfg.barbicanPackage ];
